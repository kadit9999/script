--[[ 
    ...
    Cấu hình Script: Hermanos Hub (Attack On Titan : Revolution)
]]

-- TÙY CHỈNH CHẾ ĐỘ SCRIPT TẠI ĐÂY: "PVP" hoặc "FARM"
...
local function ExecuteMainScript()
    local loadSuccess, err = pcall(function()
        -- Truyền biến chế độ script_mode toàn cục để Loader nhận diện
        getgenv().script_mode = script_mode 
...
if readfile and isfile and isfile(File_Name) then
    ...
    if savedKey == fetchedKey then
        -- Nếu key đã lưu trùng khớp với key server, tự động chạy luôn không hiện UI
        ExecuteMainScript()
        return 
    end
end
...
---------------------------------------------------------
-- KHỞI TẠO GIAO DIỆN CHUẨN (NẾU CHƯA CÓ KEY HOẶC KEY HẾT HẠN)
---------------------------------------------------------
...
SubmitBtn.MouseButton1Click:Connect(function()
    ...
    if userKey == fetchedKey then
        ...
        -- Chạy script chính Hermanos Hub
        ExecuteMainScript()
    else
